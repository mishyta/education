...
def dtest_check_sort_price_high_to_low(driver):
    ...
    page.change_dropdown_value(*MainPage.sort_dropdown, *MainPage.sort_dropdown_value, 'Цене: от высокой к низкой')
...
def Dtest_check_products_with_discount_contains_values(driver):
    ...
    page.open_page(MainPage.url)
    page.search('dress')
    cards = page.get_elements(*MainPage.product_cards)
    for card in cards:
        card = MainPage(card)
        if card.check_element_is(*MainPage.product_discount) == True:
            assert card.get_element_text(*MainPage.product_discount)[-1] == '%'
            assert card.check_element_is(*MainPage.product_regular_price) == True
