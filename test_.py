...
def test_currency_comparison(driver, value):  # 2
    ...
    with allure.step('Check that the price of products in the section "Popular goods" '
                     'is specified according to the established currency in a site header:' + value):
...
def test_total_search_products(driver):
    ...
    page.open_page(page.url)
    page.search('dress')
    with allure.step('Check that the "Search Results" page contains'
                     ' "Products: x", where x is the number of products actually found.'):
...
def test_check_product_cards_currency(driver, value):  # 6
    ...
    page.search('dress')
    page.change_page_currency(value)
    with allure.step('Сheck that the price of all shown results is displayed in:"{}"'.format(value)):
...
def test_check_sort_price_high_to_low(driver):
    ...
    with allure.step('Verifying that the items are sorted by price, '
                     'some items may be discounted, and the sorting uses the non-discounted price. '):
...
def test_check_products_with_discount_contains_values(driver):
    ...
    with allure.step('checking that for goods with a discount,'
                     ' a percentage discount is indicated along with the price before and after the discount. '):
        for card in cards:
...
def test_check_product_matching_discount(driver):
    ...
    page.open_page(page.url)
    page.search('dress')
    with allure.step('Checking that the price before and after the discount matches the specified discount amount. '):
        cards = page.get_elements(*ProductCard.locator)
        for card in cards:
            card = ProductCard(card)
            if card.check_element_is(*card.discount):
